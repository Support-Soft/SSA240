codeunit 71907 "SSAFT Export Check"
{
    ...
    procedure RunCheck(var TempErrorMessage: Record "Error Message" temporary; SAFTExportHeader: Record "SSAFT Export Header")
    var
    ...
    begin
        if SAFTExportHeader."Header Comment SAFT Type" = SAFTExportHeader."Header Comment SAFT Type"::"A - pentru declaratii anuale" then
            exit;
        ...
        if CompanyInformation."SSAFT Contact No." = '' then
            TempErrorMessage.LogMessage(
              CompanyInformation, CompanyInformation.FieldNo("SSAFT Contact No."), TempErrorMessage."Message Type"::Error,
              StrSubstNo(FieldValueIsNotSpecifiedErr, CompanyInformation.FieldCaption("SSAFT Contact No."), CompanyInformation.Name))
        else begin
    ...
    local procedure CheckCustomers(var TempErrorMessage: Record "Error Message" temporary; SAFTExportHeader: Record "SSAFT Export Header")
    var
        Customer: Record Customer;
        CustomerPostingGroup: Record "Customer Posting Group";
        CLE: Record "Cust. Ledger Entry";
        GenerateSAFTFile: Codeunit "SSAFT Generate File";
    ...
    local procedure CheckVendors(var TempErrorMessage: Record "Error Message" temporary; SAFTExportHeader: Record "SSAFT Export Header")
    var
    ...
    local procedure CheckItems(var TempErrorMessage: Record "Error Message" temporary; SAFTExportHeader: Record "SSAFT Export Header")
    var
        Item: Record Item;
        GenerateSAFTFile: Codeunit "SSAFT Generate File";
    ...
    local procedure CheckCurrencies(var TempErrorMessage: Record "Error Message" temporary; SAFTExportHeader: Record "SSAFT Export Header")
    var
