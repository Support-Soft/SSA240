...
codeunit 71903 "SSAFTSAFT Generate File"
{
    ...
    procedure ExportSingleFile(SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
        SAFTExportLine: Record "SSAFTSAFT Export Line";
        SAFTExportLine2: Record "SSAFTSAFT Export Line";
        DummySAFTExportLine: Record "SSAFTSAFT Export Line" temporary;
    ...
    local procedure ExportLine(SAFTExportHeader: Record "SSAFTSAFT Export Header"; SAFTExportLine: Record "SSAFTSAFT Export Line")
    var
    ...
    local procedure ExportHeader(SAFTExportHeader: Record "SSAFTSAFT Export Header"; SAFTExportLine: Record "SSAFTSAFT Export Line"; SingleFileExport: Boolean)
    var
        CompanyInformation: Record "Company Information";
        LinesRec: Record "SSAFTSAFT Export Line";
    ...
    local procedure ExportCompanyInfo(SAFTExportHeader: Record "SSAFTSAFT Export Header"; ParentNodeName: Text)
    var
        CompanyInformation: Record "Company Information";
    ...
    local procedure ExportGeneralLedgerAccounts(SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
    ...
    var
        ...
        SSASAFTMappingHelper: Codeunit "SSAFTSAFT Mapping Helper";
    ...
    local procedure ExportCustomers(SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
    ...
    local procedure ExportCustomer(Customer: Record Customer; SAFTExportHeader: Record "SSAFTSAFT Export Header"; _CPGCode: Code[10])
    var
        SAFTMappingRange: Record "SSAFTSAFT Mapping Range";
        CustomerPostingGroup: Record "Customer Posting Group";
        GLAccount: Record "G/L Account";
    ...
    local procedure ExportVendors(SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
    ...
    begin
        //SSM2101>>
        PurchSetup.Get;
        if PurchSetup."SSAFTExcl. Vendor Posting Grp" <> '' then
            Vendor.SetFilter("Vendor Posting Group", PurchSetup."SSAFTExcl. Vendor Posting Grp");
        //SSM2101<<
        ...
        repeat
            ...
            //SSM1724>>
            //OC ExportVendor(Vendor,SAFTExportHeader);
            VendorPostingGroup.Reset;
            //SSM2133>>
            VendorPostingGroup.SetFilter(Code, '<>%1', 'FZ_ANG');
            //SSM2133<<
    ...
    local procedure ExportVendor(Vendor: Record Vendor; SAFTExportHeader: Record "SSAFTSAFT Export Header"; _VPGCode: Code[10])
    var
        VendorPostingGroup: Record "Vendor Posting Group";
        GLAccount: Record "G/L Account";
        SAFTMappingRange: Record "SSAFTSAFT Mapping Range";
    ...
    local procedure ExportTaxTable(SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
        SAFTNAVMapping: Record "SSAFTSAFT-NAV Mapping";
        SAFTMappingRange: Record "SSAFTSAFT Mapping Range";
    begin
    ...
    local procedure ExportTaxCodeDetails(_WHTCode: Code[10]; SAFTExportHeader: Record "SSAFTSAFT Export Header"; _VAT: Boolean)
    var
    ...
    begin
        if _VAT then begin
            if _WHTCode = '' then begin
                VATPostingSetup.SetFilter("SSAFTSAFT Tax Code", '<>%1', '');
                if not VATPostingSetup.FindSet then
                    exit;
                repeat
                    ExportTaxCodeDetail(
                      VATPostingSetup."SSAFTSAFT Tax Code", VATPostingSetup.Description, VATPostingSetup."VAT %",
                      VATPostingSetup."SSAFTSAFT Deductibilitate %");
    ...
    local procedure ExportTUOMTable(SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
        SAFTNAVMapping: Record "SSAFTSAFT-NAV Mapping";
    begin
        if GuiAllowed then
    ...
    local procedure ExportAnalysisTypeTable(SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
    ...
    begin
        ...
        repeat
            ...
            if Dimension."SSAFTSAFT Export" then begin
                SafTXmlHelper.AddNewXMLNode('AnalysisTypeTableEntry', '');
                SafTXmlHelper.AppendXMLNode('AnalysisType', Dimension."SSAFTSAFT Analysis Type");
    ...
    local procedure ExportMovementTypeTable(SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
        SAFTNAVMapping: Record "SSAFTSAFT-NAV Mapping";
    begin
        if GuiAllowed then
    ...
    local procedure ExportProducts(SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
        Item: Record Item;
    ...
    local procedure ExportProduct(Item: Record Item; SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
    ...
    local procedure ExportPhysicalStockEntries(SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
        Item: Record Item;
    ...
    begin
        ...
        Location.SetRange("SSAFTSAFT Do Not Export", false);
    ...
    local procedure CalcPhysicalStock(_Item: Record Item; _SAFTExportHeader: Record "SSAFTSAFT Export Header"; _LocationCode: Code[10])
    var
    ...
    local procedure ExportAssets(SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
    ...
    local procedure ExportAsset(FixedAsset: Record "Fixed Asset"; SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
    ...
    begin
        FASetup.Get;
        if not FADepreciationBook.Get(FixedAsset."No.", FASetup."Default Depr. Book") then
            exit;
        ...
        BookValueBegin := FADepreciationBook."Book Value";
        ...
        BookValueonDisposal := FADepreciationBook."Book Value on Disposal";
        ...
        AccumulatedDepreciation := FADepreciationBook.Depreciation;

        FADepreciationBook.SetRange("FA Posting Date Filter", 0D, ClosingDate(SAFTExportHeader."Ending Date"));
        FADepreciationBook.CalcFields("Book Value");
        BookValueEnd := FADepreciationBook."Book Value";
        ...
        SafTXmlHelper.AppendXMLNode('ValuationClass', 'fiscal');
    ...
    local procedure ExportGeneralLedgerEntries(var GLEntry: Record "G/L Entry"; var SAFTExportLine: Record "SSAFTSAFT Export Line")
    var
    ...
    local procedure ExportGLEntriesBySourceCodeBuffer(var SourceCode: Record "Source Code"; var GLEntry: Record "G/L Entry"; SAFTMappingRange: Record "SSAFTSAFT Mapping Range"; SAFTExportHeader: Record "SSAFTSAFT Export Header"): Boolean
    var
    ...
    local procedure ExportGLEntriesByTransaction(var GLEntry: Record "G/L Entry"; SAFTMappingRange: Record "SSAFTSAFT Mapping Range"; SAFTExportHeader: Record "SSAFTSAFT Export Header"; TransactionDescription: Text)
    var
    ...
    begin
        repeat
            ...
            case GLEntry."Source Type" of
                ...
                GLEntry."Source Type"::Vendor:
                    ...
                    IF CustomerPostingGroup.FINDFIRST THEN BEGIN
                      ...
                      DetailedCustLedgEntry.SETRANGE("Transaction No.",GLEntry."Transaction No.");
                    ...
                    VendorPostingGroup.SETRANGE(VendorPostingGroup."Payables Account",GLEntry."G/L Account No.");
                    IF VendorPostingGroup.FINDFIRST THEN BEGIN
                      DetailedVendorLedgEntry.RESET;
                      DetailedVendorLedgEntry.SETCURRENTKEY("Transaction No.");
                      DetailedVendorLedgEntry.SETRANGE("Transaction No.",GLEntry."Transaction No.");
            ...
            ELSE
              SafTXmlHelper.AppendXMLNode('Description',GLEntry."Document No.");
            */
                ...
                if GLEntryVATEntryLink.FindSet then
                    repeat
                        ...
                        if (VATEntry.Type in [VATEntry.Type::Sale, VATEntry.Type::Purchase]) then begin
                            ...
                            //SSM2020>>
                            //OC ExportTaxInformation(GetVATTaxType(SAFTExportHeader),VATPostingSetup."SSAFTSAFT Tax Code",VATEntry.Amount);
                            if (VATEntry.Type = VATEntry.Type::Sale) and (VATEntry."Document Type" in [VATEntry."Document Type"::Invoice, VATEntry."Document Type"::"Credit Memo"]) then
                                ExportTaxInformation(GetVATTaxType(SAFTExportHeader), VATPostingSetup."SSAFTSAFT Tax Code", -VATEntry.Amount)
                            else
                                ExportTaxInformation(GetVATTaxType(SAFTExportHeader), VATPostingSetup."SSAFTSAFT Tax Code", VATEntry.Amount);
                            //SSM2020<<
                        end;
    ...
    local procedure ExportSalesDocuments(var _CustLedgerEntry: Record "Cust. Ledger Entry"; var _DetailedCustLedgEntry: Record "Detailed Cust. Ledg. Entry"; var SAFTExportLine: Record "SSAFTSAFT Export Line")
    var
        SAFTExportHeader: Record "SSAFTSAFT Export Header";
        SAFTMappingRange: Record "SSAFTSAFT Mapping Range";
        TotalRecNo: Integer;
    ...
    local procedure ExportSalesDocument(var CLE: Record "Cust. Ledger Entry"; SAFTMappingRange: Record "SSAFTSAFT Mapping Range"; SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
        Customer: Record Customer;
        CustomerPostingGroup: Record "Customer Posting Group";
        GLAccount: Record "G/L Account";
    ...
    begin
        ...
        repeat
            ...
            if CLE."Document Type" = CLE."Document Type"::Invoice then begin
                //SSM2020>>
                if SalesSetup."SSAFTSAFT Autofact Cust. No." = CLE."Customer No." then begin
                    ...
                    repeat
                        ...
                        */
                        ...
                        if (SIL."VAT Bus. Posting Group" <> '') and (SIL."VAT Prod. Posting Group" <> '') then begin
                            ...
                            ExportTaxInformation(GetVATTaxType(SAFTExportHeader), VATPostingSetup."SSAFTSAFT Tax Code", SIL."Amount Including VAT" - SIL.Amount);
                        end else begin
                            //SSM1724>>
                            //WHT Tax
                ...
                if ServIL.FindSet then begin
                    ...
                    repeat
                        ...
                        if (ServIL."VAT Bus. Posting Group" <> '') and (ServIL."VAT Prod. Posting Group" <> '') then begin
                            ...
                            ExportTaxInformation(GetVATTaxType(SAFTExportHeader), VATPostingSetup."SSAFTSAFT Tax Code", ServIL."Amount Including VAT" - ServIL.Amount);
                        end else begin
                            //WHT Tax
                            WHTTaxCode := GetTax(SAFTMappingRange.Code, GLAccount."No.", GlobalNFTType::"WHT - nomenclator");
                            if WHTTaxCode <> '' then
                ...
                if not DocumentDetailsExported then begin
                    ...
                    repeat
                        ...
                        SafTXmlHelper.AppendXMLNode('DebitCreditIndicator', 'C');
                        VATPostingSetup.Get(SIL."VAT Bus. Posting Group", SIL."VAT Prod. Posting Group");
                        ExportTaxInformation(GetVATTaxType(SAFTExportHeader), VATPostingSetup."SSAFTSAFT Tax Code", GLEntry."VAT Amount");
                        SafTXmlHelper.FinalizeXMLNode;
                    until GLEntry.Next = 0;
                    DocumentDetailsExported := true;
                end;
                    ...
                    repeat
                        ...
                        */
                        ...
                        if (SCML."VAT Bus. Posting Group" <> '') and (SCML."VAT Prod. Posting Group" <> '') then begin
                            ...
                            ExportTaxInformation(GetVATTaxType(SAFTExportHeader), VATPostingSetup."SSAFTSAFT Tax Code", SCML."Amount Including VAT" - SCML.Amount);
                        end else begin
                            //SSM1724>>
                            /*//OC
                            //WHT Tax
                ...
                if ServCML.FindSet then begin
                    ...
                    repeat
                        ...
                        if (ServCML."VAT Bus. Posting Group" <> '') and (ServCML."VAT Prod. Posting Group" <> '') then begin
                            ...
                            ExportTaxInformation(GetVATTaxType(SAFTExportHeader), VATPostingSetup."SSAFTSAFT Tax Code", ServCML."Amount Including VAT" - ServCML.Amount);
                        end else begin
                            WHTTaxCode := GetTax(SAFTMappingRange.Code, GLAccount."No.", GlobalNFTType::"WHT - nomenclator");
                            if WHTTaxCode <> '' then
    ...
    local procedure ExportPurchaseDocuments(var _VendorLedgerEntry: Record "Vendor Ledger Entry"; var _DetailedVendorLedgEntry: Record "Detailed Vendor Ledg. Entry"; var SAFTExportLine: Record "SSAFTSAFT Export Line")
    var
        SAFTExportHeader: Record "SSAFTSAFT Export Header";
        SAFTMappingRange: Record "SSAFTSAFT Mapping Range";
        VLE: Record "Vendor Ledger Entry";
    ...
    local procedure ExportPurchaseDocument(var VLE: Record "Vendor Ledger Entry"; SAFTMappingRange: Record "SSAFTSAFT Mapping Range"; SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
        Vendor: Record Vendor;
        VendorPostingGroup: Record "Vendor Posting Group";
        GLAccount: Record "G/L Account";
                    ...
                    repeat
                        ...
                        */
                        ...
                        if (PIL."VAT Bus. Posting Group" <> '') and (PIL."VAT Prod. Posting Group" <> '') then begin
                            ...
                            ExportTaxInformation(GetVATTaxType(SAFTExportHeader), VATPostingSetup."SSAFTSAFT Tax Code", PIL."Amount Including VAT" - PIL.Amount);
                        end else begin
                            //WHT Tax
                            //SSM1724>>
                            /*//OC
                    ...
                    repeat
                        ...
                        */
                        ...
                        if (PCML."VAT Bus. Posting Group" <> '') and (PCML."VAT Prod. Posting Group" <> '') then begin
                            ...
                            ExportTaxInformation(GetVATTaxType(SAFTExportHeader), VATPostingSetup."SSAFTSAFT Tax Code", PCML."Amount Including VAT" - PCML.Amount);
                        end else begin
                            //WHT Tax
                            //SSM1724>>
                            /*//OC
    ...
    local procedure ExportCLEPayment(var CLE: Record "Cust. Ledger Entry"; SAFTMappingRange: Record "SSAFTSAFT Mapping Range"; SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
        Customer: Record Customer;
        CustomerPostingGroup: Record "Customer Posting Group";
        GLAccount: Record "G/L Account";
    ...
    local procedure ExportVLEPayment(var VLE: Record "Vendor Ledger Entry"; SAFTMappingRange: Record "SSAFTSAFT Mapping Range"; SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
        Vendor: Record Vendor;
        VendorPostingGroup: Record "Vendor Posting Group";
        GLAccount: Record "G/L Account";
    ...
    local procedure ExportBALEPayment(var BALE: Record "Bank Account Ledger Entry"; SAFTMappingRange: Record "SSAFTSAFT Mapping Range"; SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
        GLAccount: Record "G/L Account";
        PaymentMethod: Record "Payment Method";
    ...
    local procedure ExportMovementOfGoods(var _ItemLedgerEntry: Record "Item Ledger Entry"; var SAFTExportLine: Record "SSAFTSAFT Export Line")
    var
        SAFTExportHeader: Record "SSAFTSAFT Export Header";
        SAFTMappingRange: Record "SSAFTSAFT Mapping Range";
        TotalRecNo: Integer;
    ...
    local procedure ExportStockMovement(var ILE: Record "Item Ledger Entry"; SAFTMappingRange: Record "SSAFTSAFT Mapping Range"; SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
        Item: Record Item;
    ...
    local procedure ExportAssetTransactions(var _FALedgerEntry: Record "FA Ledger Entry"; var SAFTExportLine: Record "SSAFTSAFT Export Line")
    var
        SAFTExportHeader: Record "SSAFTSAFT Export Header";
        SAFTMappingRange: Record "SSAFTSAFT Mapping Range";
        TotalRecNo: Integer;
    ...
    begin
        Clear(TotalRecNo);
        TotalRecNo += _FALedgerEntry.Count;
        ...
        ExportAssetTrans(_FALedgerEntry, SAFTMappingRange, SAFTExportHeader);

        FAProgress += FAProgressStep;
        if GuiAllowed() then
            Window.Update(2, FAProgress);
    ...
    local procedure ExportAssetTrans(var FALE: Record "FA Ledger Entry"; SAFTMappingRange: Record "SSAFTSAFT Mapping Range"; SAFTExportHeader: Record "SSAFTSAFT Export Header")
    begin
        repeat
            SafTXmlHelper.AddNewXMLNode('AssetTransaction', '');
            SafTXmlHelper.AppendXMLNode('AssetTransactionID', Format(FALE."Entry No."));
            SafTXmlHelper.AppendXMLNode('AssetID', FALE."FA No.");
            SafTXmlHelper.AppendXMLNode('AssetTransactionType', GetTableFieldMapping(SAFTMappingRange.Code, FALE, Format(FALE."FA Posting Type"), GlobalNFTType::"Nomenclator imobilizari"));
            SafTXmlHelper.AppendXMLNode('AssetTransactionDate', FormatDate(FALE."Posting Date"));
            SafTXmlHelper.AppendXMLNode('TransactionID', Format(FALE."Transaction No."));

            SafTXmlHelper.AddNewXMLNode('AssetTransactionValuations', '');
            SafTXmlHelper.AddNewXMLNode('AssetTransactionValuation', '');
            SafTXmlHelper.AppendXMLNode('AcquisitionAndProductionCostsOnTransaction', '0'); //todo
            SafTXmlHelper.AppendXMLNode('BookValueOnTransaction', '0'); //todo
            SafTXmlHelper.AppendXMLNode('AssetTransactionAmount', '0'); //todo
            SafTXmlHelper.FinalizeXMLNode;
    ...
    local procedure FinalizeExport(var SAFTExportLine: Record "SSAFTSAFT Export Line"; SAFTExportHeader: Record "SSAFTSAFT Export Header")
    var
        SAFTExportMgt: Codeunit "SSAFTSAFT Export Mgt.";
        TypeHelper: Codeunit "Type Helper";
    begin
    ...
    local procedure ExportEmptySections(SAFTExportLine: Record "SSAFTSAFT Export Line"; _Before: Boolean)
    var
        SAFTExportLine2: Record "SSAFTSAFT Export Line";
    begin
    ...
    local procedure ExportEmptySection(SAFTExportLine: Record "SSAFTSAFT Export Line")
    var
        SAFTExportHeader: Record "SSAFTSAFT Export Header";
    begin
        case SAFTExportLine."Type of Line" of
    ...
    procedure ExistsItemLedgerEntryPeriod(_ItemNo: Code[20]; _LocationCode: Code[10]; _SAFTExportHeader: Record "SSAFTSAFT Export Header"): Boolean
    var
    ...
    local procedure GetTax(_MappingRangeCode: Code[20]; _NavCode: Code[20]; _NFTType: Integer): Code[50]
    var
        SAFTNAVMapping: Record "SSAFTSAFT-NAV Mapping";
    begin
        SAFTNAVMapping.Reset;
    ...
    local procedure GetVATTaxType(SAFTExportHeader: Record "SSAFTSAFT Export Header"): Code[10]
    begin
        //Tax TVA
        /*
