codeunit 71902 "SSAFT Mapping Helper"
{
    // SSM2101 SSCAT 04.01.2023 SAF-T

    TableNo = "SSAFT Mapping Range";
    ...
    procedure GetDefaultSAFTMappingRange(var SAFTMappingRange: Record "SSAFT Mapping Range")
    begin
    ...
    procedure ValidateMappingRange(var SAFTMappingRange: Record "SSAFT Mapping Range")
    begin
    ...
    procedure MatchMapping(SAFTMappingRange: Record "SSAFT Mapping Range")
    begin
        if GuiAllowed then
    ...
    local procedure MatchChartOfAccountsLocal(var SAFTMappingRange: Record "SSAFT Mapping Range")
    var
        SAFTMappingValues: Record "SSAFT Mapping Values";
        SAFTNAVMapping: Record "SSAFT-NAV Mapping";
    ...
    local procedure MatchCurrenciesLocal(var SAFTMappingRange: Record "SSAFT Mapping Range")
    var
        SAFTMappingValues: Record "SSAFT Mapping Values";
        SAFTNAVMapping: Record "SSAFT-NAV Mapping";
    ...
    local procedure MatchCountriesLocal(var SAFTMappingRange: Record "SSAFT Mapping Range")
    var
        SAFTMappingValues: Record "SSAFT Mapping Values";
        SAFTNAVMapping: Record "SSAFT-NAV Mapping";
    ...
    local procedure MatchUnitOfMeasureLocal(var SAFTMappingRange: Record "SSAFT Mapping Range")
    var
        SAFTMappingValues: Record "SSAFT Mapping Values";
        SAFTNAVMapping: Record "SSAFT-NAV Mapping";
    ...
    local procedure MatchTariffNoLocal(var SAFTMappingRange: Record "SSAFT Mapping Range")
    var
        SAFTMappingValues: Record "SSAFT Mapping Values";
        SAFTNAVMapping: Record "SSAFT-NAV Mapping";
    ...
    var
        FromSAFTMappingRange: Record "SSAFT Mapping Range";
        ToSAFTMappingRange: Record "SSAFT Mapping Range";
        FromSAFTNAVMapping: Record "SSAFT-NAV Mapping";
        ToSAFTNAVMapping: Record "SSAFT-NAV Mapping";
    begin
    ...
    local procedure SetupCurrenciesForMapping(SAFTMappingRange: Record "SSAFT Mapping Range")
    var
        Currency: Record Currency;
        SAFTNAVMapping: Record "SSAFT-NAV Mapping";
        Window: Dialog;
    ...
    local procedure SetupCountriesForMapping(SAFTMappingRange: Record "SSAFT Mapping Range")
    var
        CountryRegion: Record "Country/Region";
        SAFTNAVMapping: Record "SSAFT-NAV Mapping";
        Window: Dialog;
    ...
    local procedure SetupGLAccountsForMapping(SAFTMappingRange: Record "SSAFT Mapping Range")
    var
        GLAccount: Record "G/L Account";
        SAFTNAVMapping: Record "SSAFT-NAV Mapping";
        Window: Dialog;
    ...
    local procedure SetupUnitOfMeasuresForMapping(SAFTMappingRange: Record "SSAFT Mapping Range")
    var
        UnitofMeasure: Record "Unit of Measure";
        SAFTNAVMapping: Record "SSAFT-NAV Mapping";
        Window: Dialog;
    ...
    local procedure SetupPaymentMethodForMapping(SAFTMappingRange: Record "SSAFT Mapping Range")
    var
    ...
    local procedure SetupTariffNoForMapping(SAFTMappingRange: Record "SSAFT Mapping Range")
    var
        TariffNumber: Record "Tariff Number";
        SAFTNAVMapping: Record "SSAFT-NAV Mapping";
        Window: Dialog;
    ...
    local procedure SetupSAFTValuesForMapping(SAFTMappingRange: Record "SSAFT Mapping Range"; NFTType: Integer; TableID: Integer; FieldID: Integer)
    var
        SAFTNAVMapping: Record "SSAFT-NAV Mapping";
        SAFTMappingValues: Record "SSAFT Mapping Values";
        Window: Dialog;
    ...
    begin
        Dimension.SetRange("SSAFT Analysis Type", '');
        if Dimension.FindSet then
            repeat
                Dimension.Validate("SSAFT Analysis Type", CopyStr(Dimension.Code, 1, 9));
                Dimension.Validate("SSAFT Export", true);
    ...
    procedure VerifyNoMappingDone(SAFTMappingRange: Record "SSAFT Mapping Range")
    var
        SAFTNAVMapping: Record "SSAFT-NAV Mapping";
    begin
        SAFTNAVMapping.SetRange("NFT Type", SAFTMappingRange."Chart of Account NFT");
    ...
    begin
        Dimension.Reset;
        Dimension.SetRange("SSAFT Export", true);
        if not Dimension.FindSet then
            exit;

        repeat
            if Dimension."SSAFT Analysis Type" = '' then
    ...
    begin
        if VATPostingSetup.IsEmpty then
            exit;
        VATPostingSetup.SetRange("SSAFT Tax Code", '');
        if not VATPostingSetup.IsEmpty then
            LogFieldError(
              TempErrorMessage, VATPostingSetup, VATPostingSetup.FieldNo("SSAFT Tax Code"),
              StrSubstNo(VATPostingSetupWithoutTaxCodeErr, VATPostingSetup.FieldCaption("SSAFT Tax Code")));
    end;
