codeunit 70015 "SSA CU90 Purchase-Post"
{
  ...
  InvoicePostBuffer."Dimension Set ID", PurchHeader."Reason Code");
    ...
    local procedure CU826_OnAfterPostInvPostBuffer(var GenJnlLine: Record "Gen. Journal Line"; PurchHeader: Record "Purchase Header"; GLEntryNo: Integer; var GenJnlPostLine: Codeunit "Gen. Jnl.-Post Line"; TempInvoicePostingBuffer: Record "Invoice Posting Buffer" temporary)
    var
        OldGenJnlLine: Record "Gen. Journal Line";
        VATPostingSetup: Record "VAT Posting Setup";
        FirstLineAmount: Decimal;
  ...
  TempInvoicePostingBuffer."Dimension Set ID", PurchHeader."Reason Code");
        ...
        GenJnlLine.CopyDocumentFields(OldGenJnlLine."Document Type", OldGenJnlLine."Document No.", OldGenJnlLine."External Document No.", OldGenJnlLine."Source Code", '');
        GenJnlLine.CopyFromPurchHeader(PurchHeader);
        TempInvoicePostingBuffer.CopyToGenJnlLine(GenJnlLine);

        GenJnlLine."Account Type" := GenJnlLine."Account Type"::"G/L Account";
        GenJnlLine."Account No." := TempInvoicePostingBuffer."SSA Non-Ded VAT Expense Acc 1";
        GenJnlLine."Gen. Posting Type" := GenJnlLine."Gen. Posting Type"::" ";
        GenJnlLine."Gen. Bus. Posting Group" := '';
        GenJnlLine."Gen. Prod. Posting Group" := '';
        GenJnlLine."VAT Calculation Type" := 0;
        ...
        GenJnlLine."Bal. Account No." := VATPostingSetup."Purchase VAT Account";
        GenJnlLine."Currency Code" := OldGenJnlLine."Currency Code";
        GenJnlLine.Amount := Round(TempInvoicePostingBuffer."VAT Amount" / 2, 0.01);
        FirstLineAmount := Round(TempInvoicePostingBuffer."VAT Amount" / 2, 0.01);
        GenJnlLine."Amount (LCY)" := Round(TempInvoicePostingBuffer."VAT Amount" / 2, 0.01);
        GenJnlLine."Allow Zero-Amount Posting" := true;
        GenJnlPostLine.RunWithCheck(GenJnlLine);
  ...
  TempInvoicePostingBuffer."Dimension Set ID", PurchHeader."Reason Code");
        ...
        GenJnlLine."Account Type" := GenJnlLine."Account Type"::"G/L Account";
        GenJnlLine."Account No." := TempInvoicePostingBuffer."SSA Non-Ded VAT Expense Acc 2";
        GenJnlLine."Gen. Posting Type" := 0;
        GenJnlLine."Gen. Bus. Posting Group" := '';
        GenJnlLine."Gen. Prod. Posting Group" := '';
        GenJnlLine."VAT Calculation Type" := 0;
        ...
        GenJnlLine."Bal. Account No." := VATPostingSetup."Purchase VAT Account";
        GenJnlLine."Currency Code" := OldGenJnlLine."Currency Code";
        GenJnlLine.Amount := TempInvoicePostingBuffer."VAT Amount" - FirstLineAmount;
        GenJnlLine."Amount (LCY)" := TempInvoicePostingBuffer."VAT Amount" - FirstLineAmount;
        GenJnlLine."Allow Zero-Amount Posting" := true;
        GenJnlPostLine.RunWithCheck(GenJnlLine);
        //SSA948<<
    end;
