report 70003 "SSA Close Income Statement"
{
                ...
                begin
                    ...
                    if EntryNoAmountBuf.Find('-') then
                        repeat
                            ...
                            if (EntryNoAmountBuf.Amount <> 0) or
                              ((EntryNoAmountBuf.Amount2 <> 0) and (GLSetup."Additional Reporting Currency" <> ''))
                            then begin
                                GenJnlLine."Line No." := GenJnlLine."Line No." + 10000;
    ...
    requestpage
    {
        ...
        begin
            ...
            */
            ...
            RunningDate := DMY2Date(1, Month, Year);
            EndDateReq := CalcDate('<+1M>', RunningDate);
            if not ValidateEndDate(false) then begin
    ...
    begin
        ...
        if OK then
            ...
            OK := AccountingPeriod.Get(CalcDate('<-1M>', EndDateReq + 1));
        //SSA955<<
        if OK then begin
            //SSA955>>
            /* //OC
    ...
    local
    procedure InsertFirstGenJnlLine()
    begin
        //SSA955>>
        if ((EntryNoAmountBuf.Amount > 0) or ((EntryNoAmountBuf.Amount = 0) and (EntryNoAmountBuf.Amount2 > 0))) and
           ("G/L Account"."Debit/Credit" = "G/L Account"."Debit/Credit"::Debit)
        then begin
            GenJnlLine."Account No." := "G/L Account"."SSA Closing Account";
        ...
        if ((EntryNoAmountBuf.Amount < 0) or ((EntryNoAmountBuf.Amount = 0) and (EntryNoAmountBuf.Amount2 < 0))) and
           ("G/L Account"."Debit/Credit" = "G/L Account"."Debit/Credit"::Credit)
        then begin
            GenJnlLine."Account No." := "G/L Account"."No.";
            GenJnlLine."Bal. Account No." := '';
            GenJnlLine.Validate(Amount, -EntryNoAmountBuf.Amount);
        ...
        if ((EntryNoAmountBuf.Amount <> 0) or ((EntryNoAmountBuf.Amount = 0) and (EntryNoAmountBuf.Amount2 <> 0))) and
           ("G/L Account"."Debit/Credit" = "G/L Account"."Debit/Credit"::Both)
        then begin
            GenJnlLine."Account No." := "G/L Account"."No.";
            GenJnlLine."Bal. Account No." := '';
            GenJnlLine.Validate(Amount, -EntryNoAmountBuf.Amount);
    ...
    local
    procedure InsertSecondGenJnlLine()
    begin
        //SSA955>>
        if ((EntryNoAmountBuf.Amount > 0) or ((EntryNoAmountBuf.Amount = 0) and (EntryNoAmountBuf.Amount2 > 0))) and
           ("G/L Account"."Debit/Credit" = "G/L Account"."Debit/Credit"::Debit)
        ...
        if ((EntryNoAmountBuf.Amount < 0) or ((EntryNoAmountBuf.Amount = 0) and (EntryNoAmountBuf.Amount2 < 0))) and
           ("G/L Account"."Debit/Credit" = "G/L Account"."Debit/Credit"::Credit)
        then begin
            GenJnlLine."Bal. Account No." := "G/L Account"."SSA Closing Account";
            GenJnlLine."Account No." := '';
            GenJnlLine.Validate(Amount, -EntryNoAmountBuf.Amount);
            GenJnlLine."Source Currency Amount" := -EntryNoAmountBuf.Amount2;
            GenJnlLine.Validate(Correction, false);
    ...
    begin
        //SSA955>>
        if (EntryNoAmountBuf.Amount > 0) and ("G/L Account"."Debit/Credit" = "G/L Account"."Debit/Credit"::Debit) then begin
            GenJnlLine."Account No." := "G/L Account"."SSA Closing Account";
            GenJnlLine."Bal. Account No." := "G/L Account"."No.";
            GenJnlLine.Validate(Amount, EntryNoAmountBuf.Amount);
            GenJnlLine.Validate(Correction, false);
        ...
        if (EntryNoAmountBuf.Amount < 0) and ("G/L Account"."Debit/Credit" = "G/L Account"."Debit/Credit"::Credit) then begin
            GenJnlLine."Account No." := "G/L Account"."No.";
            GenJnlLine."Bal. Account No." := "G/L Account"."SSA Closing Account";
            GenJnlLine.Validate(Amount, -EntryNoAmountBuf.Amount);
            GenJnlLine.Validate(Correction, false);
        ...
        if (EntryNoAmountBuf.Amount <> 0) and ("G/L Account"."Debit/Credit" = "G/L Account"."Debit/Credit"::Both) then begin
            GenJnlLine."Account No." := "G/L Account"."No.";
            GenJnlLine."Bal. Account No." := "G/L Account"."SSA Closing Account";
            GenJnlLine.Validate(Amount, -EntryNoAmountBuf.Amount);
            GenJnlLine.Validate(Correction, false);
