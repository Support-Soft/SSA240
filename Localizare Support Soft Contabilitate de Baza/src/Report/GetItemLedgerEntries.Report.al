report 70007 "SSA Get Item Ledger Entries"
{
    ...
    begin
        with IntrastatJnlLine do begin
            ...
            Date := "FA Ledger Entry"."Posting Date";
            "Country/Region Code" := "FA Ledger Entry"."SSA Country/Region Code";
            "Transaction Type" := "FA Ledger Entry"."SSA Transaction Type";
            "Transport Method" := "FA Ledger Entry"."SSA Transport Method";
            "Source Entry No." := "FA Ledger Entry"."Entry No.";
            Quantity := 1;
            "Document No." := "FA Ledger Entry"."Document No.";
            "Item No." := "FA Ledger Entry"."FA No.";

            "Entry/Exit Point" := "FA Ledger Entry"."SSA Entry/Exit Point";
            Area := "FA Ledger Entry"."SSA Area";
            "Transaction Specification" := "FA Ledger Entry"."SSA Transaction Specification";
            "Shpt. Method Code" := "FA Ledger Entry"."SSA Shpt. Method Code";
            ...
            Amount := "FA Ledger Entry".Amount;
            if IntrastatJnlBatch."Amounts in Add. Currency" then
                Amount := CurrExchRate.ExchangeAmtLCYToFCY(
                        "FA Ledger Entry"."Posting Date", GLSetup."Additional Reporting Currency",
                        Amount, AddCurrencyFactor);

            Validate(Amount, Abs(Amount));

            "SSA Source Type" := "SSA Source Type"::"FA Entry";

            FA.Get("FA Ledger Entry"."FA No.");
            Name := FA.Description;
            Validate("Net Weight", FA."SSA Net Weight");
            "Tariff No." := FA."SSA Tariff No.";
            Validate("Cost Regulation %", IndirectCostPctReq);
            if Type = Type::Receipt then
                "Country/Region of Origin Code" := FA."SSA Country/Region of Origin";
            Insert;
        end;
    end;
