pageextension 70009 "SSA G/L Account Card70009" extends "G/L Account Card"
{
    // SSA955 SSCAT 23.08.2019 21.Funct. Localizare inchidere lunara venituri si cheltuieli
    // SSA940 SSCAT 27.09.2019 6.Funct. corectii rulaje clasa 7-6
    // SSA978 SSCAT 08.10.2019 45.Rapoarte legale-Balanta de verificare solduri precedente
    layout
    {
        addlast(Consolidation)
        {
            group("SSA SSALocalizare")
            {
                Caption = 'Localizare';

                field("SSA Check Posting Debit/Credit"; "SSA Check Posting Debit/Credit")
                {
                    ApplicationArea = All;
                }
                field("SSA Analytic/Synth1/Synth2"; "SSA Analytic/Synth1/Synth2")
                {
                    ApplicationArea = All;
                }
                field("SSA Closing Account"; "SSA Closing Account")
                {
                    ApplicationArea = All;
                }
                field("SSA Distribute Non-Ded VAT"; "SSA Distribute Non-Ded VAT")
                {
                    ApplicationArea = All;
                }
                field("SSA Non-Ded VAT Expense Acc 1"; "SSA Non-Ded VAT Expense Acc 1")
                {
                    ApplicationArea = All;
                }
                field("SSA Non-Ded VAT Expense Acc 2"; "SSA Non-Ded VAT Expense Acc 2")
                {
                    ApplicationArea = All;
                }
            }
        }
    }
}

